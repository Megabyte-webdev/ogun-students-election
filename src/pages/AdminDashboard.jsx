...
export default function AdminDashboard() {
        ...
        cell: (row) => (
          <div className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-2">
            ...
            <button
              ...
              className="px-2 py-1 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 text-xs sm:text-sm"
            >
              Delete
            </button>
          </div>
        ...
        cell: (row) => (
          <div className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-2">
            <button
              ...
              className="text-yellow-600 hover:text-yellow-800 text-xs sm:text-sm"
            >
              Edit
            </button>
            <button
  ...
  const renderTabContent = () => {
      ...
      return (
        <div>
          <div className="flex flex-col sm:flex-row justify-between mb-4 gap-2">
            ...
            <button
              className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 text-sm sm:text-base"
    ...
    if (activeTab === "votes")
      return (
        <div className="overflow-x-auto">
          <DataTable columns={columnsMap.votes} data={tabData.votes} />
        </div>
      );
    if (activeTab === "abuse")
      return (
        <div className="overflow-x-auto">
          <DataTable columns={columnsMap.abuse} data={tabData.abuse} />
        </div>
      );
      ...
      <div className="flex flex-col lg:flex-row gap-6">
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`shrink-0 flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
                  activeTab === tab.id
                    ? TAB_COLORS[tab.color]
                    : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"
                }`}
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {[
              { label: "Total Votes", value: stats.totalVotes, icon: Vote, color: "blue" },
              { label: "Active Elections", value: stats.activeElections, icon: Calendar, color: "green" },
              { label: "Positions", value: stats.totalPositions, icon: FileText, color: "yellow" },
              { label: "Candidates", value: stats.totalCandidates, icon: Users, color: "purple" },
              { label: "Abuse Reports", value: stats.reportedAbuse, icon: ShieldAlert, color: "red" },
            ].map((s) => (
              <div
                key={s.label}
                className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 flex justify-between items-center"
              >
                <div>
                ...
                <s.icon className={`w-8 h-8 sm:w-10 sm:h-10 text-${s.color}-500`} />
              </div>
            ))}
          </div>
